  ...
  SERVERS.each do |machine|
    ...
    #   SHELL
      ...
      node.vm.provision "ansible" do |ansible|
        ...
        ansible.ask_vault_pass = true   # Prompt for vault encryption password
        if ( File.file?('ansible/vault-password.txt') )
          ansible.vault_password_file = 'ansible/vault-password.txt'
          ansible.ask_vault_pass = false
        end
        # Ansible Galaxy
        ansible.galaxy_roles_path = 'ansible/roles/:../'
        if ( File.file?('ansible/roles/requirements-vagrant.yml') )
          ansible.galaxy_role_file = "ansible/roles/requirements-vagrant.yml"
          ansible.galaxy_command = 'ansible-galaxy --force install --role-file=%{role_file} --roles-path=%{roles_path} --ignore-errors'
        end
        # Ansible Playbook
        ansible.verbose   = '' # Vagrant displays ansible commands
